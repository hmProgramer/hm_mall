...
public class GoodsService {
    ...
    public PageResult<Spu> querySpuByPage(Integer page, Integer rows, String key, Boolean saleable) {
        ...
        if (CollectionUtils.isEmpty(spuList)) {
            throw new HmException(ExceptionEnums.SPU_NOT_FOUND);
        }
        //对查询结果中的分类名和品牌名进行处理
    ...
    public List<Sku> querySkuBySpuId(Long spuId) {
        ...
        if (CollectionUtils.isEmpty(skuList)) {
            throw new HmException(ExceptionEnums.SKU_NOT_FOUND);
        }
    ...
    public void deleteGoodsBySpuId(Long spuId) {
        ...
        if (count == 0) {
            throw new HmException(ExceptionEnums.DELETE_GOODS_ERROR);
        }
    ...
    public void addGoods(Spu spu) {
        ...
        if (count != 1) {
            throw new HmException(ExceptionEnums.GOODS_SAVE_ERROR);
        }
        //插入spuDetail数据
    ...
    public void updateGoods(Spu spu) {
        ...
        int count = spuMapper.updateByPrimaryKeySelective(spu);
        if (count == 0) {
            throw new HmException(ExceptionEnums.GOODS_UPDATE_ERROR);
        }
    ...
    public void handleSaleable(Spu spu) {
        ...
        int count = spuMapper.updateByPrimaryKeySelective(spu);
        if (count != 1) {
            throw new HmException(ExceptionEnums.UPDATE_SALEABLE_ERROR);
        }
    }
    ...
    public void decreaseStock(List<CartDto> cartDtos) {
        for (CartDto cartDto : cartDtos) {
            ...
            if (count != 1) {
                throw new HmException(ExceptionEnums.STOCK_NOT_ENOUGH);
            }
        }
    }
    ...
    private void fillStock(List<Long> ids, List<Sku> skus) {
        ...
        if (CollectionUtils.isEmpty(stocks)) {
            throw new HmException(ExceptionEnums.STOCK_NOT_FOUND);
        }
